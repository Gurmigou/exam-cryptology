...
class AESAlgorithm:
    def __init__(self, master_key):
        self.num_rounds = 10
        self.key_matrices = self._expand_key(master_key)
    ...
    def _expand_key(self, master_key):
        ...
        i = 1
        while len(key_columns) < (self.num_rounds + 1) * 4:
...
def encrypt(key, plaintext):
    ...
    ciphertext = AESAlgorithm(key).encrypt(plaintext, iv)
...
def decrypt(key, ciphertext):
    ...
    return AESAlgorithm(key).decrypt(ciphertext, iv)